...
void CRnnLM::initNet()
{
    ...
    syn0=(struct synapse *)calloc(layer0_size*layer1_size, sizeof(struct synapse));	// P => Projection Matrix
    if (layerc_size==0)
	syn1=(struct synapse *)calloc(layer1_size*layer2_size, sizeof(struct synapse));	// V s(t)-> y(t)
    else {
    ...
    syn_d=(direct_t *)calloc((long long)direct_size, sizeof(direct_t));  // for direct connection. Not used in the model
...
void CRnnLM::computeNet(int last_word, int word)
{
    ...
    matrixXvector(neu1, neu0, syn0, layer0_size, 0, layer1_size, layer0_size-layer1_size, layer0_size, 0);	// basically Ws(t-1)s

    for (b=0; b<layer1_size; b++) {
        a=last_word;
        if (a!=-1) neu1[b].ac += neu0[a].ac * syn0[a+b*layer0_size].weight;					// Uw(t-1)
    }
	...
	for (a=0; a<layerc_size; a++) {
	    if (neuc[a].ac>50) neuc[a].ac=50;  //for numerical stability
    	    if (neuc[a].ac<-50) neuc[a].ac=-50;  //for numerical stability
    	    val=-neuc[a].ac;
    	    neuc[a].ac=1/(1+fasterexp(val));						// s(t+1) = sigmoid(UW(t) + Ws(t-1))
	}
    }
...
void CRnnLM::learnNet(int last_word, int word)
{
    ...
    for (c=0; c<class_cn[vocab[word].class_index]; c++) {
	a=class_words[vocab[word].class_index][c];
        neu2[a].er=(0-neu2[a].ac);						// other words in class should move toward 0
    }
    neu2[word].er=(1-neu2[word].ac);	//word part				// the real word should move toward 1
    ...
    for (a=vocab_size; a<layer2_size; a++) {
        neu2[a].er=(0-neu2[a].ac);						// by default all classes moved to 0
    }
    neu2[vocab[word].class_index+vocab_size].er=(1-neu2[vocab[word].class_index+vocab_size].ac);	//class part		// move class with word to 1
    ...
    {
	// update the error for words in class
